...
const QuizQuestion = () => {
  ...
  const {
    ...
    selectedOption,
    setSelectedOption,
    resetQuizState, // Add this method to your Zustand store
  } = useQuizStore();
  ...
  // Reset quiz state on component mount (fresh login)
  useEffect(() => {
    const initializeQuiz = () => {
      // Check if this is a fresh quiz session
      ...
      // If no saved data exists, this is a fresh session - reset everything
      if (!savedStartTime || !savedQuestionIndex || !savedAnswers) {
        // Clear any existing localStorage for this quiz
        localStorage.removeItem(`quiz-${quizId}-startTime`);
        localStorage.removeItem(`quiz-${quizId}-questionIndex`);
        localStorage.removeItem(`quiz-${quizId}-answers`);
        ...
        if (resetQuizState) {
          ...
          // Manual reset if resetQuizState method doesn't exist
          setQuestionIndex(0);
          setUserAnswers([]);
          setTimeLeft(0);
          setTimeSinceStart(0);
  ...
  // Timer initialization - ONLY after quiz metadata is loaded
  useEffect(() => {
    ...
    if (savedStartTime) {
      ...
      if (!isNaN(parsed)) {
        ...
        // If timer has expired, reset for fresh attempt
        if (remaining <= 0) {
  ...
  // Timer countdown - with better safety checks
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      useQuizStore.setState((state) => {
        ...
        if (currentTimeLeft <= 1) {
          clearInterval(interval);
          // Only submit if questions are loaded
  ...
  // Show loading until quiz is initialized
