...
const QuizQuestion = () => {
  ...
  // Lazy loading state
  const [questionsCache, setQuestionsCache] = useState({}); // Cache of fetched questions
  const [quizMetadata, setQuizMetadata] = useState(null); // Quiz title, total questions, etc.
  const [loadingQuestions, setLoadingQuestions] = useState(new Set()); // Track which questions are being fetched
  ...
  const fetchQuestion = async (index) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Store quiz metadata if not already stored
        if (!quizMetadata) {
          setQuizMetadata({
  ...
  useEffect(() => {
    if (quizId && !questionsCache[questionIndex]) {
      fetchQuestion(questionIndex);
    }
  }, [quizId]);

  // Prefetch next and previous questions
  useEffect(() => {
    ...
    // Prefetch previous question (for back navigation)
    ...
    prefetchQuestions.forEach(index => {
      setTimeout(() => fetchQuestion(index), 100); // Small delay to not block current question loading
    });
  }, [questionIndex, questionsCache, quizMetadata]);

  // Timer initialization
  useEffect(() => {
    if (!quizMetadata || isTimerInitialized) return;
  ...
  }, [quizMetadata, isTimerInitialized, quizId, setTimeLeft, setTimeSinceStart]);
  ...
  useEffect(() => {
    if (!questionsCache[questionIndex]) return;
    const savedAnswers = JSON.parse(localStorage.getItem(`quiz-${quizId}-answers`) || "[]");
  ...
  }, [questionIndex, quizId, questionsCache, setSelectedOption]);

  // Tab switch warning using react-toastify
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && questionsCache[questionIndex] && !isSubmitting) {
        incrementTabSwitch();
        if (tabSwitchCount === 0) {
  ...
  }, [questionsCache, questionIndex, isSubmitting, tabSwitchCount, incrementTabSwitch]);
  ...
  const handleNextOrSubmit = async () => {
    ...
    if (currentIndex + 1 >= quizMetadata.totalQuestions) {
      ...
      // Ensure next question is loaded before navigating
  ...
  const handlePrevious = async () => {
    if (currentIndex > 0) {
      // Ensure previous question is loaded before navigating
  ...
  const handleQuestionClick = async (index) => {
    // Ensure the clicked question is loaded before navigating
...
//   // ✅ Initialize questionIndex from URL or localStorage only once
//   useEffect(() => {
//     if (isInitialized) return;
    
//     const urlIndex = parseInt(indexParam, 10);
//     const savedIndex = parseInt(localStorage.getItem(`quiz-${quizId}-questionIndex`) || "0", 10);
    
//     // Use URL index if valid, otherwise use saved index
//     const initialIndex = !isNaN(urlIndex) ? urlIndex : savedIndex;
    
//     setQuestionIndex(initialIndex);
//     setIsInitialized(true);
//   }, [indexParam, quizId, setQuestionIndex, isInitialized]);
...
//           // Support both array and single questionData
//           let questions = quiz.questions;
//           if (!Array.isArray(questions) && quiz.questionData) {
//             questions = [quiz.questionData];
//           }
//           setQuizData({
//             quizTitle: quiz.quizTitle || quiz.title || "Quiz",
//             totalQuestions: quiz.totalQuestions || (questions ? questions.length : 1),
//             questions: questions || [],
...
//       if (typeof quizData.timeLimit === "number" && quizData.timeLimit > 0) {
...
//     return () => clearInterval(interval);
//   }, [isTimerInitialized, timeLeft, submitQuiz]);
...
//   // ✅ Update URL when questionIndex changes (but not during initialization)
//   useEffect(() => {
//     if (!isInitialized) return;
    
//     // Save to localStorage
//     localStorage.setItem(`quiz-${quizId}-questionIndex`, questionIndex.toString());
    
//     // Update URL without causing re-render loop
//     const currentPath = `/quiz/${quizId}/${questionIndex}`;
//     if (window.location.pathname !== currentPath) {
//       navigate(currentPath, { replace: true });
//     }
//   }, [questionIndex, quizId, navigate, isInitialized]);
...
//       setSelectedOption(null);
//     }
//   }, [questionIndex, quizId, quizData, setSelectedOption, isInitialized]);
...
//     try {
//       const finalAnswers = [...userAnswers];
//       if (selectedOption !== null && quizData.questions[questionIndex]) {
//         finalAnswers[questionIndex] = {
//           questionId: quizData.questions[questionIndex]._id,
//           selectedOption,
//         };
//       }
...
//       setIsSubmitting(false);
//     }
//   }, [isSubmitting, quizData, userAnswers, selectedOption, questionIndex, quizId, navigate]);
...
//   }, [quizData, isSubmitting, tabSwitchCount, incrementTabSwitch, submitQuiz]);
...
//     }
//   }, [selectedOption, userAnswers, questionIndex, quizData, setUserAnswers, setQuestionIndex, submitQuiz]);
...
//   // Show loading if not initialized or no quiz data
...
//   const { quizTitle = "Quiz", totalQuestions = 1, questions = [] } = quizData;
//   const currentQuestion = questions[questionIndex] || {};
//   const { questionText = "", imageUrl = null } = currentQuestion;
